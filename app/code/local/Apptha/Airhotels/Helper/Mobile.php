 ...
 *
 */
class Apptha_Airhotels_Helper_Mobile extends Mage_Core_Helper_Data {
    /**
    ...
    /**
     * The detection type, using static::DETECTION_TYPE_MOBILE or static::DETECTION_TYPE_EXTENDED.
     * @deprecated since version 2.6.9
     * @var string
     */
    ...
    protected static $mobileHeaders = array (
            ...
            'HTTP_PROFILE' => null,
            /**
             * Reported by Opera on Nokia devices (eg.
             * C3).
             */
     ...
     * expression defined in the constant static::VER.
     * @var array
     */
    protected static $properties = array (
            ...
            'Opera' => array (' OPR/[VER]','Opera Mini/[VER]',
                    'Version/[VER]' 
            ),
     ...
     * Must be one of static::DETECTION_TYPE_MOBILE or
     * static::DETECTION_TYPE_EXTENDED. Otherwise, nothing is set.
     * @deprecated since version 2.6.9
     * @param string $type
     * The type. Must be a static::DETECTION_TYPE_* constant. The default
     * parameter is null which will default to static::DETECTION_TYPE_MOBILE.
     */
    public function setDetectionType($type = null) {
        if ($type === null) {
            $type = static::DETECTION_TYPE_MOBILE;
        }
        if ($type != static::DETECTION_TYPE_MOBILE && $type != static::DETECTION_TYPE_EXTENDED) {
            return;
        }
    ...
    public function getMobileDetectionRulesExtended() {
        ...
        if (! $rules) {
             ...
             */
            $rules = array_merge ( static::$phoneDevices, static::$tabletDevices, static::$operatingSystems, static::$browsers, static::$utilities );
        }
        return $rules;
    }
    /**
    ...
    public function getRules() {
        if ($this->detectionType == static::DETECTION_TYPE_EXTENDED) {
            return static::getMobileDetectionRulesExtended ();
        } else {
    ...
    public function checkHttpHeadersForMobile() {
        foreach ( $this->getMobileHeaders () as $mobileHeader => $matchType ) {
            if (isset ( $this->httpHeaders [$mobileHeader] )) {
                $this->checkHttpHeaders ( $matchType, $mobileHeader );
            }
        }
        return false;
    }
     ...
     * check http headers
     */
    public function checkHttpHeaders($matchType, $mobileHeader) {
    ...
    public function __call($name, $arguments) {
        ...
        $this->setDetectionType ( static::DETECTION_TYPE_MOBILE );
        $key = substr ( $name, 2 );
     ...
     * @return mixed
     */
    protected function matchUAAgainstKey($key, $user_agent = null) {
        /**
        ...
        if (array_key_exists ( $key, $_rules )) {
            ...
            return $this->match ( $_rules [$key], $user_agent );
        }
        return false;
    }
    /**
     ...
     * @return bool
     */
    public function isMobile($userAgnt = null, $http_headers = null) {
        if ($http_headers) {
            $this->setHttpHeaders ( $http_headers );
        }
        if ($userAgnt) {
            $this->setUserAgent ( $userAgnt );
        }
        $this->setDetectionType ( static::DETECTION_TYPE_MOBILE );
     ...
     * @return bool
     */
    public function isTablet($userAgen = null, $httpHeader = null) {
        $this->setDetectionType ( static::DETECTION_TYPE_MOBILE );
        foreach ( static::$tabletDevices as $_regex ) {
            if ($this->match ( $_regex, $userAgen )) {
                return true;
            }
        }
        return false;
     ...
     */
    public function is($key, $user = null, $http = null) {
        /**
         * Set the UA and HTTP headers only if needed (eg.
         * batch mode).
         */
        if ($http) {
            $this->setHttpHeaders ( $http );
        }
        if ($user) {
            $this->setUserAgent ( $user );
        }
        $this->setDetectionType ( static::DETECTION_TYPE_EXTENDED );
        return $this->matchUAAgainstKey ( $key );
    }
    /**
     ...
     * $regex
     * @param string $userAgent            
     * @return bool search in the HTTP headers too.
     */
    public function match($regex, $User = null) {
        /**
         * Escape the special character which is the delimiter.
         */
        $regex = str_replace ( '/', '\/', $regex );
        $match = ( bool ) preg_match ( '/' . $regex . '/is', (! empty ( $User ) ? $User : $this->userAgent), $matches );
        /**
     ...
     * @return array Prepare the version number.
